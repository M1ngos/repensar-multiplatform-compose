...
import { CheckCircle2, ChevronLeft, ChevronRight, Clock, CalendarIcon } from 'lucide-react';
...
export function OnboardingWizard({ open, volunteerId, onDismiss, onComplete }: OnboardingWizardProps) {
    ...
    const [data, setData] = useState<VolunteerOnboardingUpdate>({});
    ...
    const handleNext = async () => {
        ...
        if (step < TOTAL_STEPS) {
            ...
            await handleFinish({ ...data, ...stepData });
        }
    };
    ...
    const handleFinish = async (finalData: VolunteerOnboardingUpdate) => {
        setSaving(true);
        try {
            await volunteersApi.updateOnboarding(volunteerId, {
                ...finalData,
